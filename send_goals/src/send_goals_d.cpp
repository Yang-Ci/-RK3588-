...
int main(int argc, char** argv){
    ...
    uint8_t goal_number_n =9;
    uint8_t goal_number = 9;
...
//souxun 
    goal[3].target_pose.pose.position.x = -2.10021;
    goal[3].target_pose.pose.position.y = 1.24549;
    goal[3].target_pose.pose.orientation.z = 0.891603;  
    goal[3].target_pose.pose.orientation.w = 0.452818;

    goal[4].target_pose.pose.position.x = -1.35283;
    goal[4].target_pose.pose.position.y = 0.933777;
    goal[4].target_pose.pose.orientation.z = 0.680735;  
    goal[4].target_pose.pose.orientation.w = 0.73253;

    goal[5].target_pose.pose.position.x = -1.73894;
    goal[5].target_pose.pose.position.y = 0.200759;
    goal[5].target_pose.pose.orientation.z = -0.726681;  
    goal[5].target_pose.pose.orientation.w = 0.686975;
...
//sahngpo
    goal[6].target_pose.pose.position.x = -1.70906;
    goal[6].target_pose.pose.position.y = 0.133194;
    goal[6].target_pose.pose.orientation.z = -0.0285193;  
    goal[6].target_pose.pose.orientation.w = 0.999593;// 第四个待发送的 目标点 在 map 坐标系下的坐标位置
//a-c
    goal[7].target_pose.pose.position.x =-0.0494958;
    goal[7].target_pose.pose.position.y = -0.00355166;
    goal[7].target_pose.pose.orientation.z = -0.702038;  
    goal[7].target_pose.pose.orientation.w = 0.712139;
...
//finish
    goal[8].target_pose.pose.position.x =1.91945;
    goal[8].target_pose.pose.position.y = -0.111771;
    goal[8].target_pose.pose.orientation.z = 0.699346;  
    goal[8].target_pose.pose.orientation.w = 0.714784; 
    ...
    while(goal_number )    // total is 4 goals
    {
        switch( (goal_number_n - goal_number) ){
            case 0:
                     ...
                     ROS_INFO("dddddddddddddddddddddddddddddddddddddd");
...
//             }
        ...
        if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
            ...
            if(BVS_FLAG && result_get == 0){
                if(result_t == 256){
                    std::string command = "rosrun send_goals send_goals_rotationb";
                    int result_found =  std::system(command.c_str());
                     if(result_found == 256){
                ...
                if(result_t == 512){
                    std::string command = "rosrun send_goals send_goals_rotationv";
                    int result_found =  std::system(command.c_str());
                     if(result_found == 256){
                ...
                if(result_t == 768){
                    std::string command = "rosrun send_goals send_goals_rotations";
                    int result_found =  std::system(command.c_str());
                     if(result_found == 256){
