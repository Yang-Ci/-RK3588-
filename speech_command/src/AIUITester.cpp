...
//
TEST_CALLBACK AIUITester::testCallback = nullptr;
...
void setParams()
{
	const char *setParams = "{\"audioparams\":{\"vcn\":\"x2_xiaojuan\"}}";
...
{
    std::cout << ">>>>> 麦克风已唤醒，可进行对话" << std::endl;

    std_msgs::Bool msg;
    msg.data = true;
    wakeup_pub.publish(msg);
    std::cout << "✅ /wakeup_event 话题已发布" << std::endl;
    ROS_INFO("✅ 已向 /wakeup_event 发布唤醒信号");
        ...
        if ((MsgType)pkg.type == MsgType::AIUI_MSG) 
	{
                ...
                if (content["eventType"].asInt() == 4) 
		{
	    	...
	    	cout<<"info: "<<info<<endl;
               ...
               /* int ret = system("bash -c 'source ~/ucar_ws/devel/setup.bash && roslaunch ucar_nav ucar_navigation.launch &'"); */
        ...
        else
        {
            ROS_INFO("loser");
					...
					cout << "awake_angle: wwwwwwwww"<<endl;
